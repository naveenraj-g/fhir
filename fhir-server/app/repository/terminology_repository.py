...
class TerminologyRepository:
    ...
    async def validate_org_exists(self, org_id: str) -> bool:
        """Check that an Organization with this org_id exists in the FHIR organization table."""
        from app.models.organization.organization import OrganizationModel
        async with self.session_factory() as session:
            count = await session.scalar(
                select(func.count())
                .select_from(OrganizationModel)
                .where(OrganizationModel.org_id == org_id)
            )
            return bool(count)
    ...
    async def delete_org_concept(self, concept_id: int, org_id: str) -> bool:
        async with self.session_factory() as session:
            ...
            session.add(TerminologyAuditLog(
                ...
                performed_by=concept.user_id,
                old_value={"code": concept.code, "display": concept.display, "definition": concept.definition, "org_id": concept.org_id},
            ))
    ...
    async def get_translations(
        ...
        SourceCS = TerminologyCodeSystem.__table__.alias("source_cs")
        TargetCS = TerminologyCodeSystem.__table__.alias("target_cs")
        async with self.session_factory() as session:
            stmt = (
    ...
    async def list_concept_maps(
        ...
        SrcConcept = TerminologyConcept.__table__.alias("src_concept")
        TgtConcept = TerminologyConcept.__table__.alias("tgt_concept")
        SrcCS = TerminologyCodeSystem.__table__.alias("src_cs")
        TgtCS = TerminologyCodeSystem.__table__.alias("tgt_cs")

        async with self.session_factory() as session:
            SourceConcept = TerminologyConcept
            TargetConcept = TerminologyConcept

            # Use aliased for self-join on concepts + code systems
