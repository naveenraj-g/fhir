...
# Primitive types — expanding their sub-fields is meaningless
_PRIMITIVE_TYPES = {
...
def build_type_bindings(types_bundle: dict) -> dict[str, list[tuple]]:
    ...
    for every complex-type StructureDefinition in profiles-types.json.
    """
...
def extract_bindings(
    ...
    - If it has a direct binding → include it.
    - If its declared FHIR type has entries in type_bindings → expand one level
      (Patient.address + Address.use → Patient.address.use).
    """
    ...
    for entry in resources_bundle.get("entry", []):
        ...
        for elem in sd.get("snapshot", {}).get("element", []):
            ...
            # Direct binding on this element
            binding = elem.get("binding")
            if binding:
            ...
            # Expand sub-field bindings from the element's declared FHIR type(s)
...
async def seed(db_url: str, resources_path: str, types_path: str) -> None:
    ...
    # Deduplicate — last writer wins (direct binding preferred over inherited)
...
def main() -> None:
    ...
    asyncio.run(seed(settings.FHIR_DATABASE_URL, resources_path, types_path))
