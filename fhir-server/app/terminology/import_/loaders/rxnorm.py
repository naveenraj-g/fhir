...
# Priority order for choosing display name when a concept has multiple TTY entries
...
class RxnormLoader(BaseLoader):
    ...
    async def load(self, file_path: str) -> None:
        ...
        # Collect best display per RXCUI:  {rxcui: (display, tty_priority)}
        ...
        self._log(
            f"Scanned {line_count:,} lines → {len(best):,} unique RxCUI concepts"
        )
