...
Column layout (tab-delimited):
  ...
  Description: id | effectiveTime | active | moduleId | conceptId | languageCode | typeId | term | ...
  Relationship:id | effectiveTime | active | moduleId | sourceId | destinationId | ... | typeId | ...

Description typeId:
  900000000000003001 = FSN  (Fully Specified Name — one per concept)
  900000000000013009 = Synonym

IS-A relationship typeId: 116680003
...
def _find_file(directory: str, pattern: str) -> str:
    ...
    return sorted(matches)[-1]  # take newest if multiple dates present
...
class SnomedLoader(BaseLoader):
    ...
    async def load(self, directory: str) -> None:
        ...
        with open(concept_file, encoding="utf-8") as f:
            ...
            for line in f:
                parts = line.rstrip("\n").split("\t")
                if len(parts) >= 3 and parts[2] == "1":  # active=1
                    active_ids.add(parts[0])
        self._log(f"  {len(active_ids):,} active concepts")

        # ── Step 2: load descriptions ──────────────────────────────────────────
        # FSN → primary display name  (one per concept)
        # Synonyms → terminology_concept_synonym
        self._log("Reading descriptions...")
        fsn: dict[str, str] = {}      # concept_id → FSN term
        synonyms: dict[str, list[str]] = {}  # concept_id → [synonym, ...]
        ...
        with open(desc_file, encoding="utf-8") as f:
            ...
            for line in f:
                ...
                if type_id == FSN_TYPE_ID:
                    # Strip semantic tag in parentheses: "Diabetes (disorder)" → "Diabetes"
                    display = term
