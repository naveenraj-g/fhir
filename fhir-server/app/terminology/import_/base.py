...
class BaseLoader:
    ...
    async def bulk_insert_concepts(
        ...
        for i in range(0, len(records), BATCH_SIZE):
            ...
            # $5 is the pre-computed search text — avoids asyncpg type-ambiguity
            # that occurs when the same parameter appears as both varchar (column)
            # and text (tsvector expression) in the same statement.
            expanded = [
