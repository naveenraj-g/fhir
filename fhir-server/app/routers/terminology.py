...
@router.get(
    ...
    description=(
        "Searches concept display names and definitions using PostgreSQL full-text search. "
...
@router.get(
    ...
    description="Returns all custom concepts created by the authenticated organization. Supports filtering by code system and full-text search.",
...
@router.post(
    ...
    description=(
        "Adds a custom terminology code scoped to the authenticated organization. "
    ...
    responses={
        ...
        403: {"description": "Organization context required or not found in FHIR registry"},
...
async def create_org_concept(
    ...
    if not org_id:
        raise HTTPException(status_code=403, detail="Organization context required.")
    if not await service.validate_org_exists(org_id):
        raise HTTPException(status_code=403, detail="Organization not found in FHIR registry.")
    try:
...
@router.get(
    ...
    description="Returns a custom concept by its database ID. Only returns concepts belonging to the authenticated organization.",
