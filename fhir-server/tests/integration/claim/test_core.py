"""Integration tests for /api/fhir/v1/claims endpoints."""
from app.auth.dependencies import get_current_user
from app.main import app
from tests.conftest import make_test_user

BASE = "/api/fhir/v1/claims"
...
MINIMAL = {
    ...
    "insurance": [
        {"sequence": 1, "focal": True, "coverage": "Coverage/240001"}
    ],
}
...
async def test_create_full(client):
    ...
    assert data["priority_code"] == "normal"
...
async def test_get_fhir(client):
    ...
    assert r.status_code == 200
    assert r.json()["resourceType"] == "Claim"
...
async def test_me_filters_by_user(client):
    ...
    app.dependency_overrides[get_current_user] = make_test_user(
        sub="user-a", org_id="org-a", permissions=["claim:read"]
    )
...
async def test_delete_claim(client):
    data = await _create(client)
